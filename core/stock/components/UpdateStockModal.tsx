...
export default function UpdateStockModal({ stock, onClose }: UpdateStockModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await stockService.updateStockTransaction(stock.id, formData, user.jwtToken);
      toast.custom((t) => (
        <ToastMessage
          type={typeof response === "string" ? "error" : "success"}
          title={typeof response === "string" ? "Error" : "Success"}
          message={typeof response === "string" ? response : "Stock transaction updated successfully"}
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
      ...
      >
        ...
        <h2 className="text-xl font-semibold mb-4">Update Stock Transaction</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select name="stockType" value={formData.stockType} onChange={handleChange} className="w-full px-4 py-2 bg-zinc-800 border border-zinc-700 rounded-lg">
            ...
            <div>
              ...
              <select name="subStockType" value={formData.subStockType} onChange={handleChange} className="w-full px-4 py-2 bg-zinc-800 border border-zinc-700 rounded-lg">
