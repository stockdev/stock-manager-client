...
export function ArticleActionBar({ searchTerm, setSearchTerm }: ArticleActionBarProps) {
    ...
    try {
      ...
      toast.custom((t) => (
        <ToastMessage
          type="error"
          title="Error"
          message={(error as Error).message || "Failed to delete all articles"}
          onClose={() => toast.dismiss(t)}
        />
      ));
    } finally {
  ...
  return (
    ...
    >
      ...
      <AnimatePresence>
        {isCreateModalOpen && (
          <CreateArticleModal onClose={() => setIsCreateModalOpen(false)} />
        )}
      </AnimatePresence>
      <AnimatePresence>
