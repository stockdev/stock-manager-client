...
export function CreateArticleModal({ onClose }: { onClose: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await articleService.createArticle(createForm, user.jwtToken);
      toast.custom((t) => (
        <ToastMessage
          type={typeof response === "string" ? "error" : "success"}
          title={typeof response === "string" ? "Error" : "Success"}
          message={typeof response === "string" ? response : `Article "${response.name}" created successfully!`}
      ...
      toast.custom((t) => (
        <ToastMessage type="error" title="Error" message="Failed to create article" onClose={() => toast.dismiss(t)} />
      ));
    }
  };
